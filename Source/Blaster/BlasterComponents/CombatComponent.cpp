...
void UCombatComponent::PickupAmmo(EWeaponType WeaponType, int32 AmmoAmount)
{
	...
	bool bCanReload = EquippedWeapon &&
...
void UCombatComponent::EquipWeapon(AWeapon* WeaponToEquip)
{
	...
	if (EquippedWeapon == Character->GetDefaultWeapon() && SecondaryWeapon == nullptr) //If we have only default weapon we prefer to get the new weapon immediately
	{
		EquipPrimaryWeaponAndSwapDefault(WeaponToEquip);
	}
...
void UCombatComponent::EquipPrimaryWeaponAndSwapDefault(AWeapon* WeaponToEquip)
{
	if (WeaponToEquip == nullptr) return;
	SecondaryWeapon = Character->GetDefaultWeapon();
...
void UCombatComponent::TraceUnderCrosshairs(FHitResult& TraceHitResult)
{
	...
	if (bScreenToWorld)
	{
		...
		if (Character)
		{
			float DistanceBetweenCharacterAndMuzzleFlash = 85.f;
			float DistanceToCharacter = (Character->GetActorLocation() - Start).Size();
			Start += CrosshairWorldDirection * (DistanceToCharacter + DistanceBetweenCharacterAndMuzzleFlash);
		}
